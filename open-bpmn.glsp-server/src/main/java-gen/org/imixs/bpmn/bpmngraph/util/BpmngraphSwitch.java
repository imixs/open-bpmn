...
public class BpmngraphSwitch<T> extends Switch<T> {
    ...
    protected T doSwitch(int classifierID, EObject theEObject) {
        switch (classifierID) {
            ...
            case BpmngraphPackage.SYMBOL: {
                Symbol symbol = (Symbol)theEObject;
                T result = caseSymbol(symbol);
                if (result == null) result = caseGCompartment(symbol);
                if (result == null) result = caseGShapeElement(symbol);
                if (result == null) result = caseGLayouting(symbol);
                if (result == null) result = caseGModelElement(symbol);
                if (result == null) result = caseGBoundsAware(symbol);
                if (result == null) result = caseGArgumentable(symbol);
                if (result == null) result = defaultCase(theEObject);
                return result;
            }
