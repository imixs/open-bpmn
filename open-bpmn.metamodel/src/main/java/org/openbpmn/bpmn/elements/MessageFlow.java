...
public class MessageFlow extends BPMNElementEdge {
    ...
    public MessageFlow(BPMNModel model, Element node) {
        ...
        if (model.getCollaborationElement() != null) {

            Element bpmnPlane = model.getCollaborationElement().getBPMNPlane();
            if (bpmnPlane != null) {

                bpmnEdge = BPMNModelUtil.findBPMNEdgeInPlane(model, bpmnPlane, getId());

                // bpmnEdge = (Element) model.findBPMNPlaneElement("BPMNEdge", getId());
                if (bpmnEdge == null) {
                    // create shape element
                    createBPMNEdge(bpmnPlane);
                } else {
                ...
                logger.warning("Missing BPMNPlane for collaboration element!");
            }
        } else {
            logger.warning("Missing Collaboration element!");
        }
    }
}
